...
MEETING_CHUNK_DURATION_MIN = 5  # Changed from 10 to 5 minutes
...
chunk_summaries = []  # Store individual chunk summaries
...
def generate_final_summary(chunk_summaries, max_tokens):
    """Generate a final consolidated summary from all chunk summaries"""
    global model, processor, device
    
    if not model_loaded or not chunk_summaries:
        return None
    ...
    try:
        ...
        update_status(f"Generating final consolidated summary from {len(chunk_summaries)} chunk summaries...")
        
        # Create conversation for final summary
        conversation = [
            {
                "role": "user",
                "content": [
                    {
                        "type": "text",
                        "text": (
                            "You are an expert meeting summarizer. I will provide you with multiple chunk summaries "
                            "from different parts of a meeting recording. Please create a comprehensive final summary "
                            "that consolidates all the information.\n\n"
                            "Please organize the final summary with:\n"
                            "1. Overall Meeting Overview\n"
                            "2. Key Topics Discussed (consolidated from all chunks)\n"
                            "3. Important Decisions Made\n"
                            "4. Action Items (if any)\n"
                            "5. Next Steps\n\n"
                            f"Keep the final summary within {max_tokens} tokens.\n\n"
                            "Here are the chunk summaries to consolidate:\n\n"
        ...
        final_summary = decoded_outputs[0]
        ...
        update_status(f"Error generating final summary: {str(e)}")
        return None
...
def load_model():
    ...
    update_status("Loading AI model... This may take a few minutes.")
    try:
        device = "cuda:0" if torch.cuda.is_available() else "cpu"
        repo_id = "/home/advaita/Documents/Voxtral-Mini-3B-2507"
        
        update_status(f"Loading model from {repo_id} on device {device}")
        processor = AutoProcessor.from_pretrained(repo_id)
        model = VoxtralForConditionalGeneration.from_pretrained(
            repo_id, torch_dtype=torch.bfloat16, device_map=device)
        
        model_loaded = True
        update_status("✓ AI Model loaded successfully! Ready to start transcription.")
        
        # Enable buttons after model is loaded
        widgets['start_button'].config(state=tk.NORMAL)
        widgets['load_button'].config(state=tk.NORMAL)
        ...
        widgets['load_button'].config(state=tk.DISABLED)
...
def validate_inputs():
    ...
    if not model_loaded:
        update_status("Error: AI Model not loaded yet. Please wait for model loading to complete.")
        return False
    ...
    # Visual feedback to show the value is recognized
    widgets['max_tokens_entry'].config(bg='lightgreen')
    master.after(1000, lambda: widgets['max_tokens_entry'].config(bg='white'))
    
    # You can add more validation here if needed
    update_status(f"✓ Validation passed - Using max_tokens: {max_tokens}")
    return True
...
def transcribe_audio(audio_queue_lst, all_transcript, exit_flag, max_tokens):
    ...
    global model, processor, device, model_loaded, chunk_summaries
    
    update_status("Starting transcription process...")
    
    if not model_loaded:
        update_status("✗ Error: Model not loaded. Cannot start transcription.")
        return
        ...
        while not exit_flag.is_set():
            try:
                if audio_queue_lst.empty():
                    ...
                    chunk_count += 1
                    update_status(f"Processing audio chunk {chunk_count}: {os.path.basename(file_path)}")
                    ...
                    update_status(f"Generating summary for chunk {chunk_count} with max_tokens: {max_tokens}")
                    outputs = model.generate(**inputs, max_new_tokens=max_tokens)
                    decoded_outputs = processor.batch_decode(outputs[:, inputs.input_ids.shape[1]:], skip_special_tokens=True)
                    
                    chunk_summary = decoded_outputs[0]
                    
                    print(f"\nGenerated Chunk {chunk_count} Summary:")
                    print("=" * 80)
                    print(chunk_summary)
                    print("=" * 80)
                    
                    # Store chunk summary for later consolidation
                    chunk_summaries.append(chunk_summary)
                    
                    # Display individual chunk summary
                    summary_queue_gui.put(f"--- CHUNK {chunk_count} SUMMARY ---\n\n{chunk_summary}")
                    all_transcript.append(chunk_summary)
                    update_status(f"✓ Chunk {chunk_count} processed successfully")
                    
            except Exception as e:
                print(f"Error in transcribe_audio: {str(e)}", flush=True)
                update_status(f"Error in transcribe_audio: {str(e)}")
                
        # Generate final consolidated summary when transcription is complete
        if chunk_summaries and not exit_flag.is_set():
            update_status("All audio chunks processed. Generating final consolidated summary...")
            final_summary = generate_final_summary(chunk_summaries, max_tokens)
            
            if final_summary:
                # Display final summary in the GUI
                summary_queue_gui.put(f"\n\n{'='*60}\nFINAL CONSOLIDATED SUMMARY\n{'='*60}\n\n{final_summary}")
                all_transcript.append(final_summary)
                update_status("✓ Final consolidated summary generated successfully!")
            else:
                update_status("✗ Failed to generate final consolidated summary")
...
def setup_gui():
    ...
    widgets['start_button'] = tk.Button(
        legend_frame, text="Start Meeting", command=lambda: start_meeting(), width=20, font=text_font, state=tk.DISABLED)
    ...
    widgets['load_button'] = tk.Button(
        ...
        width=20, font=text_font, state=tk.DISABLED)
...
def reset_summaries():
    """Reset chunk summaries for new session"""
    global chunk_summaries, final_summary_generated
    chunk_summaries = []
    final_summary_generated = False
    update_status("Summary collection reset for new session")
...
def start_meeting():
    ...
    # Reset summaries for new session
    reset_summaries()
    
    max_tokens = get_max_tokens()
    update_status(f"Starting meeting with max_tokens: {max_tokens}")
    update_status(f"Audio chunk duration: {MEETING_CHUNK_DURATION_MIN} minutes")
    ...
    try:
        ...
        transcription_process = mp.Process(
            ...
            args=(audio_queue, transcript_queue, exit_flag, max_tokens),
            daemon=True)
...
def load_audio_files():
    ...
    update_status(f"Expected chunk duration: {MEETING_CHUNK_DURATION_MIN} minutes each")
...
if __name__ == "__main__":
    ...
    # Load model in a separate thread to avoid blocking the GUI
