...
from tkinter import scrolledtext, filedialog, ttk  # Added ttk for dropdown
...
# UPDATED: New function to get summary mode configuration
def get_summary_mode_config():
    ...
    if mode == "Short (500 tokens)":
        max_tokens = 500
    elif mode == "Medium (1000 tokens)":
        max_tokens = 1000
    elif mode == "Long (2500 tokens)":
        max_tokens = 2500
    else:
        # Default fallback
        max_tokens = 500
...
def get_meeting_duration_config():
    ...
    try:
        ...
        if duration > 120:  # Maximum 2 hours per chunk
            return None, "Duration cannot exceed 120 minutes"
...
def validate_inputs():
    ...
    if not mode or mode == "Select Summary Mode":
        update_status("Error: Please select a summary mode from the dropdown first!")
    ...
    max_tokens = get_summary_mode_config()
    ...
    update_status(f"✓ Validation passed - Using mode: {mode} ({max_tokens} tokens)")
...
def record_audio(audio_queue, exit_flag, stop_recording_flag, pause_flag):
    ...
    try:
        ...
        # GET DYNAMIC DURATION FROM GUI
        duration_minutes, _ = get_meeting_duration_config()
        if duration_minutes is None:
            duration_minutes = MEETING_CHUNK_DURATION_MIN  # Fallback to default
...
# UPDATED: Modified transcribe_audio function to use instruction parameter
        ...
        while not exit_flag.is_set():
            try:
                if audio_queue_lst.empty():
                    ...
                    # UPDATED: Using dynamic instruction based on max_tokens
                    if max_tokens <= 500:
                            ...
                            "content": [
                                {
                                    ...
                                    f"Summarizer: after listening the audio, please understand the total audio, then only {summary_instruction} that should be organised topic-wise, with clear things, within the audio only, don't go for hallucinations and extended sentences, and what not you heard in the audio. Keep the summary within {max_tokens} tokens."
                                    )
                                },
                            ],
                        },                        
...
# UPDATED: Modified setup_gui function
def setup_gui():
    ...
    # UPDATED: Create custom styles for dropdown
    style = ttk.Style()
    ...
    # UPDATED: Replace Max Tokens entry with Summary Mode dropdown
    tk.Label(legend_frame_input, text="Summary Mode:", font=text_font).grid(row=4, column=0, sticky="e")
    summary_modes = ["Select Summary Mode", "Short (500 tokens)", "Medium (1000 tokens)", "Long (2500 tokens)"]
    ...
    tk.Label(legend_frame_input, text="Chunk Duration (min):", font=text_font).grid(row=5, column=0, sticky="e")
    ...
    tk.Label(legend_frame_input, text="(1-120 min)", font=("Arial", 10), fg="gray").grid(row=5, column=1, sticky="e")

    tk.Label(master, text="Additional Context:", font=scrolledText_font, justify="left").grid(row=6, column=0, columnspan=1)
    widgets['context_text'] = scrolledtext.ScrolledText(master, height=5, width=40, font=text_font)
...
# UPDATED: Modified start_meeting function
def start_meeting():
...
# UPDATED: Modified load_audio_files function
