...
# Dictionary-based processing functions
def process_transcription(file_path, processor, model, mode_number, chunk_type, duration_mins, is_partial, device, repo_id):
...
def process_summary(file_path, processor, model, mode_number, chunk_type, duration_mins, is_partial, device, repo_id):
    """Handle summary processing"""
    conversation = [
        {
            "role": "user",
            "content": [
...
def process_action_points(file_path, processor, model, mode_number, chunk_type, duration_mins, is_partial, device, repo_id):
    """Handle action points processing"""
    conversation = [
        {
            "role": "user",
            "content": [
...
# Dictionary mapping modes to processing functions
mode_processors = {
    ...
    try:
        ...
        while not exit_flag.is_set():
            ...
            try:
                ...
                # Use dictionary-based processing instead of if-elif-else
                try:
                    result = mode_processors[processing_mode](
                        file_path, processor, model, mode_number, 
                        chunk_type, duration_mins, is_partial, device, repo_id
                    )
                    decoded_outputs = result['outputs']
                    output_header = result['header']
                except KeyError:
