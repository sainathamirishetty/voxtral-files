...
mode_switch_queue = mp.Queue()  # NEW: Queue for mode switching
exit_flag = mp.Event()
pause_flag = mp.Event()
stop_recording_flag = mp.Event()
mode_switch_flag = mp.Event()  # NEW: Flag to signal mode switch
...
start_time = None
running = False
meeting_active = False  # NEW: Track if meeting is active
...
summary_cnt = 1
current_processing_mode = None  # NEW: Track current processing mode
...
def process_mode_changed(event=None):
    ...
    # NEW: Handle mode switching during active meeting
...
def default_state():
    ...
    widgets['process_mode_dropdown'].config(state="readonly")  # Re-enable dropdown
        ...
        while not stop_recording_flag.is_set():
            ...
            for i in range(int(RATE / CHUNK * record_seconds)):
                ...
                if mode_switch_flag.is_set() and frames:
                    ...
                    frames = []
                    chunk_start_time = time.time()
                    # Don't clear mode_switch_flag here - let processing handle it
...
def transcribe_audio(audio_queue_lst, all_transcript, exit_flag, initial_mode):
    """
    Process audio files for transcription, summarization, or action points
    with dynamic mode switching capability and proper handling of partial chunks.
    """
    update_status("Audio processing started")
    current_mode = initial_mode
    previous_mode = initial_mode  # Track previous mode for partial chunks
        ...
        while not exit_flag.is_set():
            ...
            if mode_switch_flag.is_set():
                try:
                    ...
                    previous_mode = current_mode  # Store previous mode before switching
            ...
            try:
                ...
                if processing_mode == "Transcription":
                    ...
                    # Add mode and chunk info to output
                    chunk_info = f"[{chunk_type} CHUNK - {duration_mins:.1f} mins]" if is_partial else ""
                    output_with_mode = f"[TRANSCRIPTION MODE - {datetime.now().strftime('%H:%M:%S')}] {chunk_info}\n{decoded_outputs[0]}"
                    
                    if transcript_queue_gui:
                        transcript_queue_gui.put(output_with_mode)
                    if all_transcript is not None:
                        all_transcript.append(output_with_mode)
                ...
                # ------------------ Summarization ------------------
                    ...
                    # Add mode and chunk info to output
                    chunk_info = f"[{chunk_type} CHUNK - {duration_mins:.1f} mins]" if is_partial else ""
                    output_with_mode = f"[SUMMARY MODE - {datetime.now().strftime('%H:%M:%S')}] {chunk_info}\n{decoded_outputs[0]}"

                    if summary_queue_gui:
                        summary_queue_gui.put(output_with_mode)
                    if all_minutes is not None:
                        all_minutes.append(output_with_mode)
                ...
                # ------------------ Action Points ------------------
                    ...
                    # Add mode and chunk info to output
                    chunk_info = f"[{chunk_type} CHUNK - {duration_mins:.1f} mins]" if is_partial else ""
                    output_with_mode = f"[ACTION POINTS MODE - {datetime.now().strftime('%H:%M:%S')}] {chunk_info}\n{decoded_outputs[0]}"

                    if improve_queue_gui:
                        improve_queue_gui.put(output_with_mode)
                    if all_improve is not None:
                        all_improve.append(output_with_mode)

                else:
                    update_status(f"Warning: Unknown processing mode '{processing_mode}', skipping audio chunk")
                    continue

                update_status(f"Successfully processed {chunk_type} chunk with {processing_mode} mode")
...
def setup_gui():
    ...
    # Bind dropdown change event to enable/disable summary mode and handle mode switching
    ...
    # NEW: Add status indicator for current mode
    ...
    live_improve.title("Live Action Points")
...
def start_meeting():
    global recording_process, transcription_process, summarization_process, meeting_active, current_processing_mode
    ...
    # Keep dropdown enabled for mode switching during meeting
    # widgets['process_mode_dropdown'].config(state="disabled")  # Removed this line
...
def load_audio_files():
    global recorded_audio_process, transcription_process, summarization_process, load_audio_flag, meeting_active, current_processing_mode
    ...
    widgets['pause_resume_button'].config(state=tk.NORMAL)
    widgets['stop_button'].config(state=tk.NORMAL)
    # Keep dropdown enabled for mode switching
    # widgets['process_mode_dropdown'].config(state="disabled")  # Removed this line
...
def quit_application():
    ...
    file_name = f"{write_record_to_dir}/action_points_backup_{datetime.now().strftime('%d-%m-%Y_%H-%M')}.md"
    with open(file_name, "w+") as file:
        for i, action_text in enumerate(all_improve):
            file.write(f"# ------------ action points {i} -------------- \n" + action_text + "\n\n")
