...
app = FastAPI(title="Voxtral AI Server", version="1.0.0", lifespan=lifespan)
...
async def serve_translator():
    """Serve the translator HTML page"""
    try:
        with open("translator.html", "r", encoding="utf-8") as f:
            return f.read()
    except FileNotFoundError:
        return HTMLResponse(
            content="<h1>Error: translator.html not found</h1>",
            status_code=404
        )


@app.get("/meeting-generator", response_class=HTMLResponse)
async def serve_meeting_generator():
    """Serve the meeting generator HTML page (your mom_6666.html)"""
    try:
        # You can rename mom_6666.html to meeting_generator.html
        # OR keep the original name and serve it here
        with open("meeting_generator.html", "r", encoding="utf-8") as f:
            return f.read()
    except FileNotFoundError:
        # Fallback to original filename
        try:
            with open("mom_6666.html", "r", encoding="utf-8") as f:
                return f.read()
        except FileNotFoundError:
            return HTMLResponse(
                content="<h1>Error: meeting_generator.html not found</h1>",
                status_code=404
            )
...
if __name__ == "__main__":
    ...
    if port:
        ...
        print(f"🔤 Translator:          http://localhost:{port}/translator")
        print(f"📝 Meeting Generator:   http://localhost:{port}/meeting-generator")
        print(f"{'=' * 70}")
