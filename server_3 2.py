...
# NEW: Serve HTML Client
@app.get("/", response_class=HTMLResponse)
async def serve_client():
    """Serve the HTML client interface"""
    try:
        with open("meeting_minutes_improved-7.html", "r", encoding="utf-8") as f:
            return f.read()
    except FileNotFoundError:
        return HTMLResponse(
            content="<h1>Error: client.html not found</h1><p>Please ensure client.html is in the same directory as server.py</p>",
            status_code=404
        )
...
if __name__ == "__main__":
    ...
    if port:
        print(f"\n{'=' * 60}")
        print(f"🚀 Starting Voxtral Server on http://localhost:{port}")
