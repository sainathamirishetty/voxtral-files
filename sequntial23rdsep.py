...
def stop_audio_recording():
    ...
    exit_flag.set()  # Set this early to signal all processes to stop
    ...
    def cleanup_processes():
        try:
            # Give processes a moment to see the flags and start shutting down
            time.sleep(0.5)
            
            # Terminate processes with shorter timeouts
            if load_audio_flag and recorded_audio_process and recorded_audio_process.is_alive():
                recorded_audio_process.join(timeout=2)  # Reduced timeout
                if recorded_audio_process.is_alive():
                    recorded_audio_process.terminate()
                    recorded_audio_process.join(timeout=1)

            if not load_audio_flag and recording_process and recording_process.is_alive():
                recording_process.join(timeout=2)  # Reduced timeout
                if recording_process.is_alive():
                    recording_process.terminate()
                    recording_process.join(timeout=1)

            if transcription_process and transcription_process.is_alive():
                transcription_process.join(timeout=3)  # Slightly longer for model cleanup
                if transcription_process.is_alive():
                    transcription_process.terminate()
                    transcription_process.join(timeout=1)

            # Clear queues in a non-blocking way
            def safe_clear_queue(q, queue_name):
                try:
                    count = 0
                    while not q.empty() and count < 100:  # Prevent infinite loop
                        try:
                            q.get_nowait()
                            count += 1
                        except:
                            break
                    update_status(f"{queue_name} cleared ({count} items)")
                except Exception as e:
                    update_status(f"Error clearing {queue_name}: {str(e)}")

            safe_clear_queue(audio_queue, "Audio queue")
            safe_clear_queue(audio_queue_lst, "Audio list queue")
            safe_clear_queue(unified_output_queue, "Output queue")
            safe_clear_queue(mode_switch_queue, "Mode switch queue")
            
            # Update GUI on main thread
            def update_gui_after_cleanup():
                widgets['start_button'].config(state=tk.NORMAL)
                widgets['pause_resume_button'].config(state=tk.DISABLED)
                widgets['pause_resume_button'].config(text="Pause Meeting")  # Reset text
                widgets['process_mode_dropdown'].config(state="readonly")
                live_minutes_widget['save_button'].config(state=tk.NORMAL)
                update_status("Meeting Ended Successfully.")
            
            # Schedule GUI update on main thread
            master.after(0, update_gui_after_cleanup)
            
        except Exception as e:
            # Schedule error message on main thread
            master.after(0, lambda: update_status(f"Error during cleanup: {str(e)}"))
    
    # Run cleanup in separate thread to avoid blocking GUI
...
def quit_application():
    ...
    def save_and_quit():
        try:
            ...
            for process in [recording_process, recorded_audio_process, transcription_process]:
            ...
            update_status(f"Error during quit: {str(e)}")
            ...
            master.after(0, final_quit)
    
    # Run save and quit in separate thread
    quit_thread = threading.Thread(target=save_and_quit, daemon=True)
    quit_thread.start()
