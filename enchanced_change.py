...
def update_processing_mode():
    ...
    try:
        print("Change Mode button clicked!")  # Debug
        new_mode = get_selected_processing_mode()
        print(f"Current mode: {current_processing_mode}, New mode: {new_mode}")  # Debug
        ...
        update_status(f"Error changing mode: {str(e)}")
...
def default_state():
    ...
    widgets['change_mode_button'].config(state=tk.DISABLED, bg="#cccccc", fg="black")  # NEW: Disable change mode button
...
def setup_gui():
    ...
    widgets['change_mode_button'] = tk.Button(
        ...
        font=text_font, state=tk.DISABLED, bg="#cccccc", fg="black"
    )
...
def start_meeting():
    ...
    widgets['change_mode_button'].config(state=tk.NORMAL, bg="#4CAF50", fg="white")
