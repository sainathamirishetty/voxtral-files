...
@app.get("/simple_transcription_page_2.html", response_class=HTMLResponse)
async def serve_audio_assistant():
    """Serve the Audio Assistant (simple_transcription_page.html)"""
    try:
        with open("simple_transcription_page_2.html", "r", encoding="utf-8") as f:
            return f.read()
    except FileNotFoundError:
        return HTMLResponse(
...
@app.get("/full_meeting_system_2.html", response_class=HTMLResponse)
async def serve_meeting_minutes():
    """Serve the Meeting Minutes Generator (full_meeting_system.html)"""
    try:
        with open("full_meeting_system_2.html", "r", encoding="utf-8") as f:
            return f.read()
    except FileNotFoundError:
        return HTMLResponse(
...
if __name__ == "__main__":
    ...
    if port:
        ...
        print(f"🎤 Audio Assistant:     http://localhost:{port}/simple_transcription_page.html")
