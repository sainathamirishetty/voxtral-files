...
def transcribe_audio(audio_queue_lst, all_transcript, exit_flag, initial_mode):
    ...
    with dynamic mode switching capability.
    """
        ...
        while not exit_flag.is_set():
            ...
            if mode_switch_flag.is_set():
                try:
                    ...
                    update_status(f"Processing mode switched from '{old_mode}' to '{current_mode}' at {timestamp}")
            ...
            try:
                ...
                print(f"Processing audio: {file_path} with mode: {current_mode}")

                decoded_outputs = []

                # ------------------ Transcription ------------------
                if current_mode == "Transcription":
                    ...
                    # Add mode indicator to output
                    output_with_mode = f"[TRANSCRIPTION MODE - {datetime.now().strftime('%H:%M:%S')}]\n{decoded_outputs[0]}"
                ...
                # ------------------ Summarization ------------------
                    ...
                    # Add mode indicator to output
                    output_with_mode = f"[SUMMARY MODE - {datetime.now().strftime('%H:%M:%S')}]\n{decoded_outputs[0]}"
                ...
                # ------------------ Action Points ------------------
                elif current_mode == "Action Points":
                    conversation = [
                        {
                            "role": "user",
                            "content": [
                    ...
                    # Add mode indicator to output
                    output_with_mode = f"[ACTION POINTS MODE - {datetime.now().strftime('%H:%M:%S')}]\n{decoded_outputs[0]}"
                    ...
                    update_status(f"Warning: Unknown processing mode '{current_mode}', skipping audio chunk")
                    continue

                update_status(f"Processed: {file_path} using {current_mode} mode")
